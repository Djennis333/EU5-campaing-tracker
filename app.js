...
function saveCampaign() {
    localStorage.setItem('eu5Campaign', JSON.stringify(campaignData));
    console.log('✓ Campaign saved to browser storage');
}

// Load campaign from localStorage
function loadCampaignFromStorage() {
    const stored = localStorage.getItem('eu5Campaign');
    if (stored) {
        try {
            campaignData = JSON.parse(stored);
            console.log('✓ Campaign loaded from browser storage');
            return true;
        } catch (e) {
...
function showLoginScreen() {
    ...
    const stored = localStorage.getItem('eu5CurrentUser');
    if (stored) {
        currentUser = stored;
        hideLoginModal();
        return;
...
function handleLogin(event) {
    ...
    localStorage.setItem('eu5CurrentUser', currentUser);
...
function logout() {
    if (confirm('Are you sure you want to logout?')) {
        currentUser = null;
        localStorage.removeItem('eu5CurrentUser');
        location.reload();
    }
}
...
function updatePlayerDisplay() {
    ...
    if (playerDisplay && currentUser) {
        playerDisplay.textContent = `Logged in as: ${currentUser}`;
    }
}

// ============ PERMISSION CHECKS ============
function canEditNation(nationPlayer) {
    if (!currentUser) return false;
    return currentUser === nationPlayer;
}

function canDeleteEvent(event) {
    if (!currentUser) return false;
    // Only the player who created the event can delete it
...
// Initialize the app
function initApp() {
    // Try to load from storage, if fails load initial data
    if (!loadCampaignFromStorage()) {
        loadInitialData();
    }
...
function loadInitialData() {
    ...
    // Load events with createdBy tracking
    campaignData.events = [
        {
            id: 1,
            date: '1338',
        ...
        <div class="event-list-item">
            <div>
                ...
                <span style="color: var(--color-text-secondary); font-size: var(--font-size-sm);">
                    ...
                    ${event.createdBy && event.createdBy !== 'System' ? ` • By: ${event.createdBy}` : ''}
                </span>
            </div>
            <div>
                ...
                ${canDelete ? `<button class="btn btn-secondary" style="padding: var(--space-4) var(--space-12); background-color: #8B0000;" onclick="deleteEvent(${event.id})">Delete</button>` : ''}
            </div>
        </div>
    `}).join('');
}
            ...
            ${sortedEvents.map(event => {
                ...
                <div class="timeline-event">
                    ...
                    <div style="margin-top: var(--space-12); display: flex; gap: var(--space-8);">
                        ...
                        ${canDelete ? `<button class="btn btn-secondary" style="padding: var(--space-4) var(--space-8); font-size: 12px; background-color: #8B0000;" onclick="deleteEvent(${event.id})">Delete</button>` : ''}
                    </div>
                </div>
            `}).join('')}
        </div>
...
function renderNations() {
    ...
    nationsGrid.innerHTML = campaignData.nations.map(nation => {
        ...
        const canEdit = canEditNation(nation.player);
            ...
            <div class="nation-card" style="border-left: 4px solid ${nation.color};">
                <div class="nation-header">
                    ...
                    <div class="nation-info">
                        <h3>🏰 ${nation.name}</h3>
                        <p class="nation-player">👤 Player: ${nation.player}
                            ${currentUser === nation.player ? ' (Your Nation)' : ''}
                        </p>
                    </div>
                </div>
                ...
                <div class="nation-section">
                    ...
                    <p id="goals-${nation.name}">${nation.goals}</p>
                    ${canEdit ? `
                        <button class="btn btn-secondary" onclick="editNationGoals('${nation.name}')">Edit Goals</button>
                    ` : ''}
                </div>
...
function editNationGoals(nationName) {
    const nation = campaignData.nations.find(n => n.name === nationName);
    if (!nation || !canEditNation(nation.player)) {
        alert('You can only edit your own nation!');
        return;
    }
    
    const newGoals = prompt(`Edit goals for ${nationName}:`, nation.goals);
    if (newGoals !== null && newGoals.trim()) {
        nation.goals = newGoals.trim();
        saveCampaign();
        renderNations();
        alert('✓ Goals updated and saved!');
    }
}
...
function generateChronicle() {
    ...
    sortedEvents.forEach(event => {
        chronicleHTML += `
            ...
            ${event.createdBy && event.createdBy !== 'System' ? `<p><em>Recorded by: ${event.createdBy}</em></p>` : ''}
        `;
    });
...
function openAddEventModal() {
    if (!currentUser) {
        alert('Please log in first to add events!');
        return;
    }
...
function openAddMechanicModal() {
    if (!currentUser) {
        alert('Please log in first to add mechanics!');
        return;
    }
...
function addEvent(event) {
    ...
    const newEvent = {
        id: Math.max(...campaignData.events.map(e => e.id), 0) + 1,
...
function showEventDetail(eventId) {
    ...
    contentEl.innerHTML = `
        <div style="margin-bottom: var(--space-16);">
            ...
            ${event.createdBy ? `<p><strong>📝 Recorded by:</strong> ${event.createdBy}</p>` : ''}
        </div>
        <div style="margin-bottom: var(--space-16);">
        ...
        ${canDelete ? `
            <div style="margin-top: var(--space-16);">
                <button class="btn btn-secondary" style="background-color: #8B0000;" onclick="deleteEvent(${event.id}); closeModal('eventDetailModal')">Delete Event</button>
            </div>
        ` : ''}
    `;
